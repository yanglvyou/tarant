...
export default function List(props: Props) {
  ...
  const styleObject = Object.assign(getStyleObj(props, active), props.customStyle);
  const colorStyleObject = getColorStyleObject(props);
  const { leftIcon, leftIconSize, title, subTitle, desc, rightIcon, className } = props;
    ...
    >
      ...
      {(title || subTitle) && (
        <View className="list__box">
          {title && <View className={classNames('list__title', 'title-class')}>{title}</View>}
          {subTitle && (
            <View
      ...
      {rightIcon && (
        <View className="list__rightIcon">
          <OsIcon size={28} type={rightIcon} customStyle={{ verticalAlign: 'top' }}></OsIcon>
        </View>
      )}
    </View>
  );
