...
const needTestWhiteList = [
  ...
  'packages/taro-vantify-demo/src/components/**/*.tsx',
  'packages/taro-vantify-demo/src/components/**/*.ts',
  'packages/taro-vantify-demo/src/pages/**/**',
...
const modifyNeedTestFileList = modifyFileList.filter(item => needTestFileList.includes(item));

if (modifyNeedTestFileList.length) {
  console.log(`👓 修改了${modifyNeedTestFileList.length}个核心 ui 组件文件，运行测试任务`);
  const { code } = exec(`npx pnpm --filter=taro-vantify-demo test:background`);
  process.exit(code);
} else {
